...
// so the CCG pipeline needs its own driver. Reads LLVM IR, runs codegen, writes
...
int main(int argc, char **argv) {
  cl::ParseCommandLineOptions(argc, argv, "CCG code generator\n");

  LLVMInitializeCCGTargetInfo();
  LLVMInitializeCCGTargetMC();
  LLVMInitializeCCGTarget();
  LLVMInitializeCCGAsmPrinter();
  ...
  const Target *T = TargetRegistry::lookupTarget("ccg", TT, Error);
  if (!T) {
    errs() << "error: " << Error << "\n";
    return 1;
  }
