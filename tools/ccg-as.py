#!/usr/bin/env python3
"""
Minimal assembler for CCG, driven entirely by the TableGen description.
...
Usage: ccg-as.py <ccg.json> <input.s> <output.bin>
"""
import json, re, sys
...
def load(jsonpath):
    ...
    for k, v in recs.items():
        if not (isinstance(v, dict) and "Inst" in v and isinstance(v.get("Size"), int)):
            continue
        if v["Size"] == 0 or "CCGInst" not in v.get("!superclasses", []):
            continue
        ops, tied = [], set()
...
def main():
    ...
    try:
        ...
        print(f"ccg-as: {e}", file=sys.stderr)
        return 1
