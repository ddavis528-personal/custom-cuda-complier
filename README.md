...
| [`docs/benchmarks.md`](docs/benchmarks.md) | Instruction density and instruction count against five AMD GPU generations and PTX, measured rather than cited, normalized to work done as well as per warp, with the limits of each comparison stated. Every table is regenerated by `tools/verify.sh`. |
...
Measured against five AMD GPU generations — GCN5 through RDNA4 and CDNA3 — the
encoding runs 26–30 bits per instruction against their 38–49, and AMD's density
is flat across those eight years, so the margin is not an artifact of an old
baseline. Instruction counts are comparable; normalized to *work done* rather
