//===-- CCGMaskUniform.cpp - run warp-uniform work on one lane ------------===//
//
// O-33. Measured with `-ccg-uniformity-stats`, half of what an addressing-heavy
...
#define DEBUG_TYPE "ccg-mask-uniform"

static cl::opt<bool> EnableMasking("ccg-mask-uniform", cl::init(true),
                                   cl::desc("run warp-uniform work on lane 0 "
                                            "and broadcast (O-33)"));
static cl::opt<bool> ReportMasking("ccg-mask-stats",
...
unsigned predicatedForm(unsigned Op) {
  switch (Op) {
  case CCG::ADD:   return CCG::ADD_P;
  case CCG::SUB:   return CCG::SUB_P;
  case CCG::AND:   return CCG::AND_P;
  case CCG::OR:    return CCG::OR_P;
  case CCG::XOR:   return CCG::XOR_P;
  case CCG::SHL:   return CCG::SHL_P;
  case CCG::SHR:   return CCG::SHR_P;
  case CCG::SRA:   return CCG::SRA_P;
  case CCG::MIN_S: return CCG::MIN_S_P;
  case CCG::MIN_U: return CCG::MIN_U_P;
  case CCG::MAX_S: return CCG::MAX_S_P;
  case CCG::MAX_U: return CCG::MAX_U_P;
  case CCG::MUL_HI_S: return CCG::MUL_HI_S_P;
  case CCG::MUL_HI_U: return CCG::MUL_HI_U_P;
  case CCG::MADLO: return CCG::MADLO_P;
  case CCG::FADD:  return CCG::FADD_P;
  case CCG::FSUB:  return CCG::FSUB_P;
  case CCG::FMUL:  return CCG::FMUL_P;
  ...
  case CCG::LD_GLOBAL: return CCG::LD_GLOBAL_P;
  default:         return 0;
  }
}
...
bool isDivergentSource(const MachineInstr &MI) {
  return MI.getOpcode() == CCG::SRD && MI.getOperand(1).isImm() &&
         MI.getOperand(1).getImm() == 0;
}

class CCGMaskUniform : public MachineFunctionPass {
public:
  static char ID;
  CCGMaskUniform() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override {
    return "CCG warp-uniform lane-0 masking";
  }
};

char CCGMaskUniform::ID = 0;

} // namespace

bool CCGMaskUniform::runOnMachineFunction(MachineFunction &MF) {
  if (!EnableMasking)
    return false;
  ...
  const unsigned QualTrue = unsigned(CCG::P3 - CCG::P0);

  MachineBasicBlock &Entry = MF.front();
  BuildMI(Entry, Entry.begin(), DebugLoc(), TII->get(CCG::PMOV_IMM), CCG::P3)
...
namespace llvm {
FunctionPass *createCCGMaskUniform() { return new CCGMaskUniform(); }
} // namespace llvm
