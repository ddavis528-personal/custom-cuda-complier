...
enum class Mask {
  ...
  NoPredForm,
  /// The ISA gives it a predicated encoding; CCGInstrInfo.td has no `_P` twin
...
Mask classify(const Instruction &I) {
  ...
  switch (I.getOpcode()) {
  // Points 0-25 and 32-63, all with `_P` twins in CCGInstrInfo.td. `mul`
  ...
  // tens of bytes; CCGMaskUniform does the real check.
  ...
  // §4 point 19: the qualifier is the selector. See SEL in CCGInstrInfo.td.
  ...
  bool runOnFunction(Function &F) override {
    ...
    static const Row Rows[] = {
        ...
        {Mask::NoMIForm, "no MI form",
         "ISA predicates it, CCGInstrInfo.td does not"},
    ...
    // CCGMaskUniform runs post-ISel, sees real opcodes and real displacements,
    // and declines any region whose masked work does not exceed its crossings.
    // "maskable" above is the ceiling, not the take.
           << "    (ceiling, at IR level -- CCGMaskUniform decides the take;"
              " ccg-sim -counters measures it)\n";
    return false;
  }
