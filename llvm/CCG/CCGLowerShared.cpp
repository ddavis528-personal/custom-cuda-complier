//===-- CCGLowerShared.cpp - assign addresses to .shared globals ---------===//
//
...
#define DEBUG_TYPE "ccg-lower-shared"
...
bool runOnMod(Module &M) {
  ...
  for (Function &F : M)
    if (!F.isDeclaration())
      F.addFnAttr("ccg-shared-bytes", std::to_string(Offset));
  return true;
}

class CCGLowerShared : public ModulePass {
public:
  static char ID;
  CCGLowerShared() : ModulePass(ID) {}
  bool runOnModule(Module &M) override { return runOnMod(M); }
  StringRef getPassName() const override { return "CCG .shared layout"; }
};

} // namespace

char CCGLowerShared::ID = 0;

namespace llvm {
ModulePass *createCCGLowerShared() { return new CCGLowerShared(); }
} // namespace llvm
