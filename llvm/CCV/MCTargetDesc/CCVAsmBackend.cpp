...
class CCVAsmBackend : public MCAsmBackend {
public:
  CCVAsmBackend() : MCAsmBackend(llvm::endianness::little) {}

  unsigned getNumFixupKinds() const override {
    return CCV::NumTargetFixupKinds;
  }
  ...
  const MCFixupKindInfo &getFixupKindInfo(MCFixupKind Kind) const override {
    ...
    static const MCFixupKindInfo Infos[CCV::NumTargetFixupKinds] = {
        // name                    offset  size  flags
        {"fixup_ccv_bra21",        11,     21,   MCFixupKindInfo::FKF_IsPCRel},
        {"fixup_ccv_brapred18",    0,      32,   MCFixupKindInfo::FKF_IsPCRel},
        {"fixup_ccv_call17",       15,     17,   MCFixupKindInfo::FKF_IsPCRel},
        {"fixup_ccv_bra8",          8,      8,   MCFixupKindInfo::FKF_IsPCRel},
    };
  ...
  void relaxInstruction(MCInst &Inst,
                        const MCSubtargetInfo &) const override {
    assert(Inst.getOpcode() == CCV::C_BRA && "nothing else relaxes");
    Inst.setOpcode(CCV::BRA);   // same single operand, 16 bits -> 32
  }
...
class CCVELFObjectWriter : public MCELFObjectTargetWriter {
public:
  CCVELFObjectWriter()
...
void CCVAsmBackend::applyFixup(const MCAssembler &, const MCFixup &Fixup,
  ...
  if (Kind == CCV::fixup_ccv_bra8) {
    const int64_t D8 = branchDisplacement(Value, /*InstrSize=*/2);
    if (!isInt<8>(D8))
      report_fatal_error("CCV: bra.short target out of range -- relaxation "
  ...
  switch (Kind) {
  case CCV::fixup_ccv_bra21:
    if (!isInt<21>(D))
      report_fatal_error("CCV: bra target out of range (±2 MB, §3 Format E)");
    Word |= uint32_t(D & 0x1fffff) << 11;
    break;

  case CCV::fixup_ccv_brapred18: {
    ...
    if (!isInt<18>(D))
      report_fatal_error("CCV: bra.pred target out of range (±256 KB, §3)");
  ...
  case CCV::fixup_ccv_call17:
    if (!isInt<17>(D))
      report_fatal_error("CCV: call target out of range (±128 KB, §3)");
    Word |= uint32_t(D & 0x1ffff) << 15;
    break;

  default:
    report_fatal_error("CCV: unknown fixup kind");
  }
...
namespace {
class CCVAsmBackendWithWriter : public CCVAsmBackend {
public:
  std::unique_ptr<MCObjectTargetWriter>
  createObjectTargetWriter() const override {
    return std::make_unique<CCVELFObjectWriter>();
  }
};
} // namespace

MCAsmBackend *llvm::createCCVAsmBackend(const Target &, const MCSubtargetInfo &,
                                        const MCRegisterInfo &,
                                        const MCTargetOptions &) {
  return new CCVAsmBackendWithWriter();
}
