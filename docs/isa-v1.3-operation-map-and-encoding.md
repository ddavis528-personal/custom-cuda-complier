...
Version 1.2 was specified from the inside out — operations, formats, density — and never
crossed the boundary between a kernel and the runtime that launches it. First-pass
compiler work crossed it immediately, and 1.3 is mostly the consequence. Six of the eight
changes below came from attempting to lower the simplest possible CUDA kernel and finding
that it had no encoding.

*Addition.* **§5, execution environment and launch ABI.** Previously absent entirely. The
address model, the launch block, entry register state, and `srd` — the one value a kernel
cannot obtain from memory. The V1.2 note carrying "special-register / launch-ABI surface"
as a deferred, non-encoding-blocking item was wrong: it blocked the first instruction of
the first kernel.

*Addition.* **Address model — `(rbase << 16) + roffset`.** V1.2 never stated an address
width. The widest value a GPR lane can hold is 32 bits, which caps the address space at
4 GB and cannot be reconciled with 64-bit device pointers. Resolved by keeping 64-bit
width out of every register and letting it exist only in the computed effective address.
See §5.1 and O-17.

*Addition.* `srd rd, #sel` (Format K), the thread- and CTA-identity primitive. One opcode
point, sixteen selectors. See §5.3 and O-18.

*Addition.* `ld.pred` / `st.pred` (Format D) with a 4-bit predicate mask. V1.2 had no
path from memory to a predicate and no path from a GPR to a predicate, so the predicate
file could be spilled but never reloaded — a register class a compiler cannot spill is one
it can only fail on. See O-19.

*Addition.* `pand` / `por` / `pxor` / `pmov` (Format K), predicate-to-predicate logic.
V1.2 had no predicate-to-predicate operation of any kind. See O-20.

*Addition.* `unballot pd, rs` (Format G). O-14's deferred companion to `ballot`, now
added — its trigger condition fired. See O-14.

*Addition.* `packi.z` (Format B), a clearing variant of `packi`, so the head of a
full-lane compose chain carries no false dependency on its destination. See O-21.

*Respecified.* **Partial-write semantics are now stated once, as invariant 10.** V1.2
specified preservation for `packi` and left it unstated for predicated writes to a
predicate destination, which if-conversion cannot be built against either way.

*Correction.* Format D carried **two contradictory opcode maps**, the second of which
allocated `opcode[0]` as a predication bit — contradicting invariant 4, O-1, and the
existence of the D/D′ tag pair. It predated O-1's resolution and has been deleted.
...
- 16 vs. 32 GPRs; 4 predicate registers — both pending compiler spill data, with the GPR
  prior now moved by the four arguments in §1. The predicate count is the one that changed
