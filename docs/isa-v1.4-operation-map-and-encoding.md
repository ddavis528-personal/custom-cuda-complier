...
All bit maps in §3 have been checked for field-width mismatch, overlap and gaps. Two
parameters remain provisional pending first-pass compiler data and are marked as such in
§1. Open items are in §8; §9 is the decision log recording what was settled and why.

**Changes since 1.3.**

1.3 came from attempting to lower a kernel. 1.4 comes from transcribing §3 into a machine
description — a TableGen target description that an assembler, an encoder and a
disassembler are generated from. Four of the five changes below were invisible in prose and
surfaced only because a bit map that reads correctly has to be exactly right in a form a
machine consumes.

*Respecified.* **Formats B′ and B″ move the predicate qualifier to the canonical `[29:27]`,
and B″ its predicate destination to `[31:30]`,** with the immediate split around the
qualifier as D′ and `bra.pred` already do. They were the only exceptions to invariant 8
among the enumerated formats, and they were exceptions on the one field that feeds the
predicate RAT — forcing exactly the format-dependent mux invariant 8 exists to prevent. No
immediate loses a bit, and B″ becomes layout-identical to C′. See O-22.

*Corrected.* **Invariant 8 described a compressed geometry shared by classes J and K.** No
such shared geometry exists, and cannot: Format J carries three register fields in the twelve
bits after the class code and subop, so its `rd` sits at `[7:4]` where K's sits at `[11:8]`.
Each class is internally fixed, which is what the rename path requires; the claim that they
agree was simply wrong.

*Corrected.* **Invariant 8's "no exceptions among the 32/48-bit formats" was wider than its
own enumeration.** Format I has never conformed and should not — `pmov` puts `pd` at `[9:8]`
and `chwidth.multi` has no canonical slots. The enumeration is the rule; the blanket
sentence is gone.

*Clarified.* **Format G is written out as four field layouts rather than one table with "or"
...
prices it correctly — the compressed forms cannot address 32 registers without a second
encoding.
...
|---|---|---|
| Logical GPRs | 16 (`R0`–`R15`), 4-bit field | provisional — see the note below |
...
**On the GPR count.** Still provisional, but the evidence has stopped being neutral.
Four independent arguments now point at 32, and only the last is visible in spill counts:

1. A four-register group at 16 GPRs leaves exactly four legal aligned destinations, each a
   quarter of the architectural file — the structural reason span was deferred (§10).
2. Because `chwidth` transitions drain, an allocator wants to partition the file softly by
   width; at 16 GPRs a two-width kernel gets eight registers per width.
...
**And it is an encoding fork, not a parameter.** Widening the register field from 4 to 5 bits
...
needs. So **the compressed forms cannot address a 32-register file**, and with them goes the
density argument in §6.

The resolution, if 32 is chosen, is the one RISC-V uses: compressed forms address a
16-register subset while the 32-bit formats widen. That is a genuine second encoding, and it
gives the register allocator a third objective — prefer the low sixteen for
compressed-eligible operations — competing with destructive-form preference (O-8) and width
affinity. Spill counts remain a flag flip; a working 32-GPR machine does not. Price both.
...
- 16 vs. 32 GPRs; 4 predicate registers — both pending compiler spill data, with the GPR
  prior now moved by the four arguments in §1 and its cost repriced: per §1 it is a second
  encoding rather than a parameter, since the compressed forms cannot address 32 registers. The predicate count is the one that changed
  character in 1.3: with O-19 and O-14 the file is spillable, so 4 is a performance
  parameter that data can settle, rather than a hard ceiling on if-conversion depth that
  would have to be set conservatively. **Carry one forward risk into the GPR decision:** span transfers were deferred partly because a
