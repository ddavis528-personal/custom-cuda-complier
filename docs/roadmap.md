...
| F-104 Format B has 28 free opcode points and the compressed format has the immediates | Step 6 | **open — written up for ISA review in `proposals/immediate-operands.md`.** Format B is the 32-bit register-immediate format with a 5-bit opcode: **32 points, four used** (`addi` at 0, `packi`/`packi.z`/`unpacki` at 20–22). The **16-bit** Format K meanwhile carries eight immediate ALU forms — but two-address (`rd = rd op #imm`) and `uimm4` only, so an operation is encodable there only when the destination *is* the source and the constant is 0–15. Outside that box there is no register-immediate form at any length. Proposed: **Format B point *n* is Format A point *n* with the second source replaced by the immediate** — already true of `addi`/`add`, so it is a rule rather than a new table, and the Format B map becomes a projection of §4's. Seven points cover the measured need; nine applies the rule whole and leaves 19 free. `packi`/`unpacki` at 20–22 sit where the rule would put `abs`/`neg`/`popc`, which are unary and have no immediate form, so the rule is well-formed |
...
| F-108 Three instructions available with no ISA change at all | Step 6 | **open, compiler-side.** (1) **Define `mul.lo` at §4 point 2** — the ISA allocates a three-address multiply and `CCVInstrInfo.td` has only `C_MUL_LO` (compressed, two-address) and `MADLO` (three-source), so every `a * b` with a distinct destination becomes `movi rZ, 0` + `mad.lo`. One `def`. (2) **Select the immediate compare** — `SETP_EQ_I` and siblings exist and are generated; ISel materialises the constant instead. Two instructions. (3) **Select `C_ANDI`/`C_SHRI` where they fit** the two-address `uimm4` box. These need no decision from the ISA side |
...
| F-106 NVIDIA ships the warp-uniform register file this project keeps arguing about | Step 6 | **open — O-25 and F-52 now have a shipping precedent.** The SASS carries a **uniform register file**: `S2UR UR4, SR_CTAID.X` reads the CTA index into one register for the whole warp rather than 32 copies, `ULDC` loads constants into it, and ordinary vector instructions take `UR` operands beside vector ones (`IMAD R9, R9, UR4, R0`). That is precisely what O-25 argued from register-file size and F-52 from redundant execution, and it is in the compatibility target. **O-33's lane-0 masking is this project's software approximation of it** — `transpose` spends three `shfl.idx` broadcasts recovering what a uniform register supplies for free — which reframes the masking work as a workaround rather than an optimisation, and strengthens the case for the hardware |
