...
1. **F-106 — the warp-uniform register file.** The decision, and the evidence is
   now in. F-129 measured the three shapes and only one supports it: a
   grid-strided fused elementwise chain, whose divergent working set is **four
   values at any tensor count** while its uniform working set crosses the
   16-entry file at around eight fused tensors and spills from there. Neither
   argument previously made for a uniform file survived measurement —
   `gpr-count-decision.md`'s GEMM accumulator pressure does not bind until 4×4
   (F-113), and F-128's GEMM address pressure turned out to be **divergent**
   addressing, which a uniform file cannot hold (retracted). So the question to
   decide is narrower and sharper than it was: *is the fused elementwise
   epilogue a shape CCV should be good at?* If yes, the case is strong and
   specific. If the answer is GEMM, a uniform file is not the lever.

2. **F-126 — tie the two liveness measurements together.** Partly overtaken:
   F-130's divergence split answered the question F-126 was posed to answer, and
   the answer was that the uniform and spilled values are *not* the same set in
   `sgemm`. What remains is the same correspondence for the fused case — the
   uniform peak is an IR-level ceiling, and the spill classifier works on
   physical registers after allocation. The F-129 conclusion is robust to that
   gap (a divergent working set of 4 cannot be what spills 79 times) but the
   *size* of the win is not yet pinned.
...
| F-131 Accumulator capacity: what a uniform file cannot do, and what tuning can | Step 6 | **measured.** Two questions, taken together. (1) **Can a warp-uniform register file hold accumulators? No, and not for an implementation reason.** A GEMM accumulator is `C[row0+i][col0+j]` where `row0` and `col0` derive from `threadIdx`: every lane holds a *different* element, which is what data-parallel accumulation means. A uniform register holds one value shared by 32 lanes. The two are incompatible by definition, so no uniform file of any size or design helps here. It helps only indirectly, by vacating whatever GPRs addressing was occupying — and F-129 measured `sgemm`'s divergent working set at 69 values against a 16-entry file at 2×4, so vacating ~10 uniform ones does not change the outcome. (2) **The accumulator problem is also smaller than it looked**: at every tile the machine can practically run, accumulator spill is 0, 0, 4, 8 of 34, 54, 85, 139 transfers. The cliff is at 4×4 and beyond. What binds below it is divergent index arithmetic. **The largest measured lever is not the ISA at all — it is how the kernel is written.** `sgemm.cu` carries `#pragma unroll` on every inner loop, which is the idiom of a 255-register machine; no cost model can override a pragma. Sweeping the unroll factor at 2×2: **294 instructions and 85 spill transfers at full unroll, 207 and 29 at unroll=4, 173 and 27 at unroll=2**. That is static code and static spill; dynamic traffic needs the simulator and has not been run, so the size of the win is not yet established — only its direction and that it is large. **And the lever is currently blocked**: at unroll=1, and at unroll≤2 for larger tiles, the per-thread staging arrays stay as allocas and the backend aborts with "Do not know how to expand the result of this operator!". §5.1 windows `.local` and O-30 reserves the frame pointer, so the addressing model exists; the legalization path does not. `CCVCheckIR` now diagnoses it in a sentence instead of crashing, and `test/reject/local-array.ll` pins it. This blocks any tuned GEMM library (backend-context Phase 4), which uses local arrays routinely |
