...
| F-53 O-24's manufactured predicate costs an instruction per compare | Step 6 | open — no hardwired always-true predicate (§1), so every compare is `por pd,!pd,pd` then `@pd setp`. Two of roughly thirty instructions in the division sequence. AMD's compare writes an implicit condition register that `s_cselect` reads at no extra cost. Not a headline, but a real recurring cost of a settled decision, and compare-dense code is where it shows |
