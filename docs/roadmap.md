...
| F-56 Conversions and SFU cannot be predicated | Step 6 | **mostly resolved — O-34.** Conversions moved from §4 128+ into 64–127, which is inside Format A′'s 7-bit opcode, and gained predicated twins. `transpose`'s "no A′ form" bucket went 3 → 1 and its lane-activations 1809 → 1747. **What remains is `rcp.f32`**: the SFU is at 256+, A′ reaches 127, and the 128–255 range O-34 freed is Format A only — so relocating the SFU down one range buys nothing; it has to come below 128, where the only space left is the six free integer points at 26–31 and the eight spare in `dp`'s new 48–63 home. Open, one instruction wide |
