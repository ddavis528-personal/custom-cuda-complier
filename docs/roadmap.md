...
| F-104 Format B has 29 free opcode points and the compressed format has the immediates | Step 6 | **open, and it is the cheapest instruction-count win available.** Format B is the 32-bit register-immediate format with a 5-bit opcode — **32 points, three used**: `addi`, `packi`, `unpacki`. The **16-bit** Format K meanwhile carries eight immediate forms: `C_ANDI`, `C_ORI`, `C_XORI`, `C_SHLI`, `C_SHRI`, `C_SRAI`, `C_SUBI`, `C_ADDI`. So `rd = rd & 15` is one 16-bit instruction while `rd = rs & 15` is **two 32-bit ones**, for want of an opcode point in a format with 29 free. Adding `andi`/`ori`/`xori`/`shli`/`shri`/`srai`/`subi` costs seven points and removes three `movi` from the benchmark as it stands; the zero register is a separate and larger win. Both need an encoding-owner decision, not a compiler change |
