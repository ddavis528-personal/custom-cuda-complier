...
| F-87 Width transitions are loop-carried and never amortize | Step 6 | **open, and it is the largest measured payoff in the benchmark.** `vadd16_loop`'s steady-state body narrows `r6`/`r7` at the top and widens `r6` back at the bottom, every iteration, so that the width state at the back-edge matches loop entry. Nothing in the loop needs `r6` wide; the restore exists only to satisfy a fixed point the dataflow picked. Had it picked "narrow at loop entry", both transitions hoist into the preheader. `CCVInsertChwidth` cannot: it hoists within a block and stops at block entry, because "hoisting across a block boundary would need the dataflow to agree on every predecessor, which is a separate problem". Costless in a straight-line kernel; in a loop it converts a one-time cost into a per-iteration one. **Measured: the i16 loop body is 9 instructions against fp32's 7, and 4 of the 9 are narrow element work — a 44% narrow fraction. At O-40's 2× that is 7.0 cycles, exactly break-even. Hoisting the two transitions gives a 7-instruction body and 5.0 cycles: 1.40× per element.** The ISA side works; the compiler is giving the entire benefit back. This is a compiler limitation, not an ISA property — `chwidth` here is loop-invariant and §3 does not prevent hoisting it |
