...
| F-59 §4's `cvt` block has a count but no arithmetic | Step 6 | open — proposal in `proposals/conversion-encoding.md`. **First framing was wrong** and said §4 had no encoding for an integer source: the 2-bit format code selects an interpretation to be read at the register's `chwidth`, not a width, and codes `10`/`11` are unallocated at every `chwidth`, so an integer source is an allocation rather than a mechanism. What survives is that §4 fixes the block's *shape* (8 bases × 4 source codes × 4 rounding = 128) and never its *arithmetic* — O-28 made numbering normative and gave rules for the two ranges that needed one, and this is a third. So there is no fact of the matter about what `cvt.f32.u32` is, and `CCVInstrInfo.td` assigned 128–131 flat. The proposal also notes that collapsing the eight destination bases into a 2-bit destination code halves the block to 64 points, which is exactly the A′-reachable range 64–127 and would resolve F-56 — at the cost of `dp4`/`dp8`'s predicated encoding |
