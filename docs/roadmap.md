...
| F-66 Narrow→wide extension has no lowering | Step 6 | **half resolved by O-38.** `zext` is now free — widening a register clears the exposed bits, which is exactly what a zero-extension is, so it costs no masking instruction beyond the `chwidth` already needed. `sext` is still open: zeros do not replicate a sign, so it needs `shl`+`sra` or a dedicated point, and nothing selects it today |
