...
| F-147 A by-value parameter struct was a silent miscompile | Step 6 | **fixed, with the half that cannot be fixed here diagnosed instead.** Asking whether `multi_tensor_apply` has to be hand-written turned up two defects behind it. **(1)** clang passes `__global__ void k(..., Params p)` as `ptr byval(%struct.Params)` -- the struct's BYTES are in the parameter space and the pointer is a fiction of the calling convention. `CCVLowerKernelArgs` tested `isPointerTy()`, so it classified the struct as a window index, spent a launch slot on it, and read every field from whatever window the struct's first four bytes happened to name. **It compiled, assembled and round-tripped**; a parameter struct is an ordinary CUDA idiom and nothing static could see it, which makes this worse than the crashes in F-142 and F-144. Lowered correctly now: the object IS the launch block's bytes at its offset, so the argument becomes a constant pointer there and each field access folds into base+displacement like the scalar slots beside it. `tools/check-byval.sh` executes it with field values chosen so a regression cannot pass by luck -- `n` is 5 and the input tensor is poked into window 5, so a kernel that misread the struct as a window index would read plausible floats and produce plausible answers. **(2)** The struct-of-pointers case -- which is what `multi_tensor_apply` actually is -- loads a pointer out of memory, and aborted with "Do not know how to expand this operator's operand". The invariant-11 group in `CCVCheckIR` covered a pointer compared, a pointer phi, a pointer passed across a call and a pointer STORED; **reading one back was the direction nobody wrote a rule for**, and it is the direction that occurs, because a kernel indexing a pointer table never stores one. Now diagnosed, with `test/reject/ptr-loaded.ll`. The diagnosis is the whole fix available here: §5.1 gives an address a window and an offset and invariant 11 keeps the pair out of a register, and nothing says what that pair looks like as a value IN memory. Settling it is an ISA question -- `proposals/pointer-representation.md` |
