...
| F-78 No benchmark exercises v1.6's own headline feature | Step 6 | **open.** `chwidth` and the sub-32-bit element model (O-6, O-38, F-3) are most of what v1.6 added, and **none of the five benchmark kernels touches it** — all are fp32/i32. `test/bench/vadd16.cu` exists and the chwidth work measures it, but it is absent from the density and instruction-count tables, so those tables say nothing about the feature. Every AMD generation swept in F-74 has packed 16-bit math, so the comparison is available and has simply not been made |
