...
| F-55 `shfl` is predicated-only, so a broadcast needs a manufactured guard | Step 6 | open — §3 says Format G carries "predicated forms; unpredicated live in K", but Format K has `vote` and `ballot` and no shuffle. Any lane-0-broadcast scheme (see `proposals/warp-uniform.md`) would reintroduce exactly the O-24 tax that O-32 removed from compares |
