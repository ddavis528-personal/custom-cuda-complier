...
2. **F-106 — the warp-uniform register file.** The decision. Evidence is now in
   (F-113, F-126, and `proposals/warp-uniform.md` §0's SASS comparison), and it
   is a stronger case than `gpr-count-decision.md`'s own framing: the decision
   named a uniform file as the escape hatch *if accumulator pressure binds*,
   and the measurement says accumulator pressure does not bind until 4×4 while
   address pressure binds everywhere.
...
4. **F-116 — remove the dead predicated-compare path.** 21 instructions that
   O-32 superseded, declared rather than deleted so removal could have its own
   regression run.
