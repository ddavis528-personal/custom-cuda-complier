...
| F-68 The ISA does not say what width an addressing index is read at | Step 6 | open — §3 says the base+index scale comes from `rdata`'s `chwidth` and says nothing about the width the **index register itself** is read at. Invariant 11 and §5.1 imply 32 bits (an index is an address component, and no register holds an address), but it is not stated. It stopped being academic when the allocator assigned one register as both a 16-bit destination and a 32-bit index in the same instruction: `chwidth r2, 1; ld.global r2, [r3, r2, 0, 0]`. Worked around with `@earlyclobber` on the narrow indexed forms, at the cost of one extra register per narrow load — which is itself a data point for F-3's width-affinity argument. **Needs a sentence in §3** |
| F-69 `chwidth.multi` merging recovers nothing on a real kernel | Step 6 | open, measured — O-6 justifies Format I's register mask by "a kernel entering a packed section typically reconfigures several at once", and the pass merges consecutive same-width `chwidth` runs. On the hand-written test that collapses 3 into 1; on `vadd16` it merges **0 of 3**, because the transitions are not adjacent — each sits before the load that needed it. O-6's benefit needs the **hoisting** §3 actually asks for ("hoist the mask instruction to a point where the affected registers are cold"), and hoisting is bounded by where each register is dead. Adjacent-run merging is the cheap half and it is not the half that pays |
