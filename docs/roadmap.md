...
| F-56 Conversions and SFU cannot be predicated | Step 6 | open — §4 gives Format A′ a 7-bit opcode, so points 128+ and 256+ are Format A only. The division sequence's `cvt` and `rcp` therefore run in all 32 lanes however uniform they are, which is the largest single block on O-33's coverage. Moving them into A′'s reach means finding 128 points in a range that is full |
| O-33 lane-0 masking | Step 6 | **done — `ccg-mask-uniform`, on by default, `-ccg-mask-stats` to report. Result-identical by construction and checked as such: `tools/check-mask.sh` runs the same kernel masked and unmasked against an independent reference and fails if nothing was masked, so it cannot pass vacuously. Measured −14% lane-activations on a uniform-chain microbenchmark and −21.5% on `transpose`** |
