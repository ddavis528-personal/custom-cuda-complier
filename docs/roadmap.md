...
1. **F-113 — instrument GEMM spill by cause.** The measurement
   ...
   **except that F-111 found `dp4.acc` is not selected at all**, so see item 3.
...
3. **F-121 — select the Format J accumulate forms**, `dp4.acc` first. This moved
   up because F-111 found it unselected: `gpr-count-decision.md` credits
   `dp4.acc` as one of the two reasons 16 GPRs survive INT8 GEMM accumulator
   pressure, and the compiler cannot emit it. **An INT8 spill measurement taken
   today measures a machine without that mitigation**, so this either lands
   before F-113 reports or F-113's INT8 column has to say it is missing.

4. **F-119 — a predicate broadcast, so O-33 can gate a compare.** Lane-0 masking
...
| F-116 The predicated compare path is dead code, and looked live | Step 6 | **open — cleanup, deliberately deferred.** O-32 added Format C″, the unpredicated compare, and ISel now selects it directly. The ten `PSEUDO_SETP_*` nodes and the `CCVExpandPseudos` arms that lower them predate that; nothing emits the pseudos, so the ten predicated Format C forms they expand to are dead with them, 21 instructions in total including `PSEUDO_PTRUE`. **This is the transitive case that the first version of the F-111 checker got wrong:** the expansion arms are textbook emit sites, so a one-hop check certifies the whole path as reachable. Declared `dead` rather than deleted, because removal touches the pseudo-expansion pass for no measured benefit and belongs in its own change with its own regression run. The audit did fix one real hazard in passing: the inner opcode switch ended in `default: Opc = CCV::SETP_NE`, which covered `PSEUDO_SETP_NE` by accident and would have turned any pseudo later added to the outer case list into a not-equal compare silently |
...
| F-121 `dp4.acc` is credited by the GPR decision and not selected | Step 6 | **open, and it has a live consequence.** All five Format J accumulate forms — `ffma.acc.f0`, `ffma.acc.f1`, `dp4.acc`, `mad.acc` — and the three-source `dp4.ss` have no pattern. `gpr-count-decision.md` credits `dp4.acc` as one of the two mitigations that make 16 GPRs sufficient under INT8 GEMM accumulator pressure, on the grounds that it does four MACs per accumulator register. **That credit is unearned while nothing selects it**, which bears directly on F-113: an INT8 spill measurement taken today measures a machine without the mitigation the decision assumes. Either the pattern lands before F-113 reports, or F-113's INT8 column has to say so |
