...
| F-30 `bar.wait`'s phase parity is an immediate, but must alternate per dynamic execution | Step 4 | **open — NEEDS AN ISA DECISION. The barrier's epoch parity flips on each completion, so a barrier executed N times needs N alternating values out of one encoded bit. O-12's "the compiler alternates the bit each time through the loop" is not implementable: an immediate is fixed at assembly time. Correct only where the barrier runs at most once per kernel. Diagnosed (`test/reject/barrier-in-loop.ll`), not miscompiled. Blocks the Step 4 reduction kernel. See `proposals/barrier-phase.md`** |
| F-31 Shared memory (`addrspace(3)`) has no load/store selection | Step 4 | open — `ld.shared`/`st.shared` base+offset exist in the MC layer; base+index forms, shared-symbol addressing and a `.shared` layout do not. Diagnosed as "cannot select", not miscompiled |
