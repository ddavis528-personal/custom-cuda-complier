...
1. **F-126 — close the last gap before the warp-uniform call.** F-113 reported,
   and it moved the argument: pointer/index spill dominates at every tile that
   fits 16 GPRs, and peak warp-uniform values live is 11 at every tile size.
   The two numbers are measured at different points in the pipeline, though —
   `unif` is an IR-level ceiling, the spill classifier runs on physical
   registers after allocation — so they are not yet shown to be the same eleven
   values. That correspondence is what turns "a uniform file would hold roughly
   this much" into "a uniform file would remove this much spill", and it is the
   one measurement left.

2. **F-106 — the warp-uniform register file.** The decision. Evidence is now in
   (F-113, F-126, F-127, F-128, and `proposals/warp-uniform.md` §0's SASS
   comparison), and it needs both halves of F-128 put to it together: the case
   *for* a uniform file is stronger than `gpr-count-decision.md`'s own framing,
   since address pressure binds at every tile CCV can run while accumulator
   pressure does not bind until 4×4 — and a uniform file nonetheless does
   **nothing** for the tile ceiling F-127 measures, because accumulators are
   divergent. Those are separate arguments and only one of them is what a
   uniform file answers.
...
| F-128 The spill cause crosses over at 4×4, which bounds what a uniform file can do | Step 6 | **open — this is the sharpest thing F-113's instrumentation found, and it cuts against the obvious reading of F-126.** Spill by cause does not scale uniformly; it **changes character**. At 1×1–2×4 it is almost all pointer/index (22, 42, 66, 110 against accumulator 0, 0, 4, 8). At 4×4 the two are level (146 against 148). At 8×8 accumulator spill is **1164 of 1491, 78%**, and pointer/index has risen only to 294. Peak warp-uniform values live is **11 at every tile** — it never moves, because it is a property of the addressing. **A warp-uniform register file holds uniform values, and accumulators are divergent by construction** — they are per-thread C elements. So a uniform file would relieve the spill that dominates at every tile CCV can currently run, and would do **nothing** for the tile-size ceiling that F-127 measures, because that ceiling is set by divergent accumulator capacity. Upper bound on the relief, taking all pointer/index spill as removable: 2×4's `sp/mac` goes 2.07 → 0.43. That is a large win for the tiles CCV has and it does not move CCV toward 8×8. **Both halves of that belong in the F-106 decision**, because the case for a uniform file and the case against the register file being large enough are separate arguments and only one of them is what a uniform file answers |
